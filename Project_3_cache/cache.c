...
int main(int argc, char * argv []) {
 ...
 // Process the command line arguments
  ...
  int cacheLines = (cachesize_kb * 1000)/blocksize_bytes; //This gives us the amount of cache lines needed for each way
  ...
  int indexTopBit = offsetBits + indexBits - 1;
  ...
  while (scanf("%c %d %lx %d\n", &marker, &loadstore, &address, &icount) != EOF)
  {
    ...
    int index = (address & (((1U << indexBits) - 1) << offsetBits));
